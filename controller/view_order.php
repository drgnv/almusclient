...
//print_r($Basic->getActiveDriversCount());
...
if(isset($_GET['order_id'])){
    $_SESSION['current_order_id'] = $_GET['order_id'];
   $os=$Basic->checkOrderStatus($_GET['order_id']);
   $user_l = $Basic->getUserById($_SESSION['user_info'][0]['user_id']);
 //  print_r($Basic->haversineGreatCircleDistance(
  //$os['driver_info'][0]['lat'], $os['driver_info'][0]['lon'], $user_l[0]['lat'], $user_l[0]['lon'], $earthRadius = 6371));
// print_r($os);
   //echo '<br>'.$os['driver_info'][0]['lat'].'-'.$os['driver_info'][0]['lon'].'<br><br>'.$user_l[0]['lat'].'-'.$user_l[0]['lon'];
    $Smarty->assign('os', $os);
  $Smarty->assign('u', $user_l);
